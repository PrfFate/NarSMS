  ...
  void _showDeleteDialog(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (BuildContext dContext) {
        return AlertDialog(
          title: const Text('Silme İşlemi', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red)),
          content: const Text('Bu cihazı sistemsel olarak silmek istediğinize emin misiniz? Bu işlem geri alınamaz!'),
          actions: [
            TextButton(
    ...
    return BlocConsumer<DeviceBloc, DeviceState>(
      ...
      builder: (context, state) {
        ...
        bottom: PreferredSize(
          ...
          child: Container(color: const Color(0xFFF57C00), height: 2.0), // Cihaz sayfaları için turuncu şerit
        ),
      ),
            ...
            _buildCard(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  if (device.features != null && device.features!.isNotEmpty) ...[
            ...
            _buildCard(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  FutureBuilder(
                    future: getIt<GetDeviceMovementsUseCase>().call(device.id),
                    builder: (context, snapshot) {
                      ...
                      if (snapshot.hasError) {
                        return const Center(
                          child: Text('Hareketler yüklenirken bir hata oluştu.', style: TextStyle(color: Colors.red)),
                        );
                      }
                      ...
                      return result.fold(
                        (failure) => Center(child: Text(failure.message, style: const TextStyle(color: Colors.red))),
                        (movements) {
                          if (movements.isEmpty) {
                            return const Center(
                              child: Padding(
                                padding: EdgeInsets.all(16.0),
                                child: Text('Cihaza ait hareket kaydı bulunmuyor.', style: TextStyle(color: Colors.grey)),
                              ),
                            );
                          }
      ...
      children: [
        Column(
          children: [
            Container(
              ...
              child: const Icon(Icons.compare_arrows, size: 16, color: Color(0xFFF57C00)),
            ),
            Container(
              width: 2,
              height: 40,
        ...
        Expanded(
          child: Column(
            ...
            children: [
              Text(
                mov.movementType ?? 'Bilinmeyen Hareket',
              ...
              const SizedBox(height: 2),
              Text(
                mov.movementDate != null ? DateFormat('dd.MM.yyyy HH:mm').format(mov.movementDate!) : '-',
    ...
    return Row(
      ...
      children: [
        Container(
          ...
          child: Icon(icon, size: 20, color: iconColor ?? const Color(0xFFF57C00)),
        ),
        const SizedBox(width: 16),
        Expanded(
          child: Column(
