...
class _DeviceEditPageState extends State<DeviceEditPage> {
  ...
  void initState() {
    ...
    _serialNumberController.text = widget.device.deviceSerialNumber ?? '';
    if (widget.device.purchasePrice != null) {
      _purchasePriceController.text = widget.device.purchasePrice.toString();
    }
    _selectedDeviceType = widget.device.deviceTypeName;
    _selectedSupplier = widget.device.supplierName;
    
    if (widget.device.purchaseDate != null) {
      _isPastDate = true;
      _selectedDate = widget.device.purchaseDate;
    }

    if (widget.device.features != null) {
      for (var f in widget.device.features!) {
        var val = f.featureValue;
        switch (f.featureName?.toLowerCase()) {
          case 'ram': _selectedRam = val; break;
          case 'hafıza': 
            _selectedHafiza = val; 
            _selectedHardDiskHafiza = val; // aynı alan
            break;
          case 'ekran': _selectedEkran = val; break;
          case 'i̇şlemci': _selectedIslemci = val; break;
          case 'port': _selectedPort = val; break;
          case 'güç': _selectedUpsGuc = val; break;
        }
      }
    }
  }
  ...
  void _onSave() {
    if (_formKey.currentState!.validate()) {
      ...
      final request = {
        ...
        "status": widget.device.status ?? 'InStock',
        "purchaseDate": purchaseDate.toIso8601String(),
        "purchasePrice": purchasePrice,
        "features": features,
      };

      context.read<DeviceBloc>().add(UpdateDevice(id: widget.device.id, requestData: request));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Cihazı Düzenle', style: TextStyle(color: Colors.black87, fontWeight: FontWeight.bold)),
                ...
                child: Container(
                  ...
                  child: ElevatedButton(
                    ...
                    child: isLoading 
                      ...
                      : const Text('Cihazı Güncelle', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                  ),
                ),
              ),
