...
/// Kısmi kargolanan satışlar sayfası.
class PartiallyShippedSalesPage extends StatefulWidget {
  const PartiallyShippedSalesPage({super.key});

  @override
  State<PartiallyShippedSalesPage> createState() => _PartiallyShippedSalesPageState();
}

class _PartiallyShippedSalesPageState extends State<PartiallyShippedSalesPage> {
  static const int _pageSize = 20;
  static const String _status = 'PartiallyShipped';
    ...
    return Card(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return _SalePartiallyShippedCard(sale: sales[index], onRefresh: _load);
        },
      ),
    );
  }
  ...
  Widget _buildEmpty() => Card(
    ...
    child: const Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      SizedBox(height: 16),
      Text('Kısmi kargolanan satış bulunamadı', style: TextStyle(fontSize: 14, color: Colors.grey)),
    ])),
  );
...
class _SalePartiallyShippedCard extends StatelessWidget {
  final SaleEntity sale;
  final VoidCallback onRefresh;
  const _SalePartiallyShippedCard({required this.sale, required this.onRefresh});

  @override
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () async {
        final result = await Navigator.pushNamed(context, AppRouter.shippedSaleDetail, arguments: sale);
