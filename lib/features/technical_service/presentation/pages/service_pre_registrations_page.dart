...
class _ServicePreRegistrationsPageState
  ...
  void initState() {
    ...
    // Page is wrapped with BlocProvider in home_page, but to be sure we just use context.read
    _loadRequests();
  }
              ...
              child: BlocConsumer<TechnicalServiceBloc, TechnicalServiceState>(
                ...
                builder: (context, state) {
                  ...
                  if (state is TechnicalServiceLoaded) {
                    ...
                    return _buildRequestList(state.requests, state.hasMore, state.isLoadingMore);
                  }
                  if (state is TechnicalServiceError) {
    ...
    return Card(
      ...
      child: const Center(
        child: Column(
          ...
          children: [
            Icon(Icons.assignment_outlined, size: 64, color: Color(0xFF3F51B5)),
            SizedBox(height: 16),
            Text(
              'Servis ön kaydı bulunmuyor',
          ...
          return InkWell(
            onTap: () async {
              final result = await Navigator.pushNamed(
                context,
                AppRouter.servicePreRegistrationDetail,
                arguments: request,
              );
                  ...
                  Column(
                    ...
                    children: [
                      if (request.requestDate != null)
                        Text(
                          DateFormat('dd.MM.yyyy').format(request.requestDate!.toLocal()),
    ...
    return Container(
      ...
      child: PopupMenuButton<String>(
        ...
        onSelected: (value) async {
          if (value == 'Yeni Ön Kayıt Ekle') {
            final result = await Navigator.pushNamed(
              context,
              AppRouter.servicePreRegistrationAdd,
            );
            if (result == true) _loadRequests();
          }
        },
        itemBuilder: (BuildContext context) => [
          PopupMenuItem<String>(
            value: 'Yeni Ön Kayıt Ekle',
            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
            child: menuItem(Icons.add_circle_outline, 'Yeni Ön Kayıt Ekle'),
          ),
        ],
      ),
    );
